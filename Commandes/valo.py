...
@commands.check(
    lambda ctx: checks.hasRolePerm(ctx, ctx.guild.id, ctx.author.id, "Valorant")
)
async def valorantGroup(ctx: commands.Context):
    ...
    if not ctx.invoked_subcommand:
        ...
        await ctx.send_help(valorantGroup)


@commands.command(name = "lier", aliases = ["l"])
async def lierCmdValorant(ctx: commands.Context, compte: str, serveur = "europe"):
...
async def créerCmdPractice(ctx: commands.Context):
    """Permet aux membres autorisés de créer une practice"""
...
valorantGroup.add_command(lierCmdValorant)
