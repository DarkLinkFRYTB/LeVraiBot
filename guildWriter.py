...
def findUser(userID: int, guildID: int | str) -> dict | None:
...
def dataGuild(guildID: int, arg: commands.Bot | commands.Context):
    """Vérifie et réécris certaines données du serveur dans le fichier JSON si elles sont incorrectes"""
    with open("./guilds.json") as file:
        def verifyRecursive(data: dict | list, base: dict | list, sendDel: bool = False) -> bool | None:
            ...
            if isinstance(data, dict):
                ...
                for element in list(data):
                    ...
                    if element not in base:
                        ...
                        if isinstance(data[element], dict) | isinstance(data[element], list):
                ...
                for element in data:
                    ...
                    #Effectue la fonction récursive plus loin dans l'arborescence si l'élément est un objet ou une liste
                    if isinstance(element, dict) | isinstance(element, list):
